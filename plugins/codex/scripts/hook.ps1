...
# $env:ROGUE_LOG_FILE → $env:ROGUE_LOG_DIR/codex.log → default.
...
if (-not $logFile) {
    ...
    if ($logDir) { $logFile = Join-Path $logDir 'codex.log' }
}
...
function Log {
    ...
    try {
        ...
        Add-Content -LiteralPath $logFile -Value "$stamp provider=codex event=$EventName $Msg" -Encoding UTF8
    } catch {}
}
